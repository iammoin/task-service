...
from models.users import UserTokenData, UserModel
from models.exceptions import AuthException
...
async def build_request_context(request: Request,
    ...
    if user_data_from_context:
        from repository.users import UserRepository
        user_repository: UserRepository = UserRepository(input_db=db)
        user: UserModel = user_repository.get_by_username(user_data_from_context.username)
        error_message = None
        if not user:
